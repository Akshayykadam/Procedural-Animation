...
// Represents the state where the character is touching an object/environment
public class TouchState : EnviormentInteractionState
{
    // Constructor to initialize the state with its context and key
    public TouchState(EnviormentInteractionContext context, EnviormentInteractionStateMachine.EEnviormentInteractionState estate) : base(context, estate)
    {
    ...
    {
        // Return the current state key, as TouchState does not transition to another state
        return StateKey;
    }
